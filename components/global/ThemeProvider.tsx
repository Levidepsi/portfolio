...
export const ThemeProvider = ({ children, navigation }: { children: React.ReactNode, navigation: any }) => {
	...
	return (
		<div>
			<div className={`h-[100vh] pb-[50px] overflow-y-scroll overflow-x-hidden w-full bg-[#EFEBE5] menu  ${openMenu ? " slideDown" : ""}`}>
				<div className="flex justify-between mx-5 py-5 border-solid border-[#30282A] border-b-[1px]">
					<Link href={"/"}>
							<Image src={navigation.header_logo2} alt="Header 2" width={500} height={500} className="h-auto w-full firstimage max-w-[156px] " />
					</Link>
						 <div className="x-button" onClick={() => setOpenMenu(false)}></div>
				</div>
				<div className="mt-[45px] mx-5">
					{navigation.header_menu && navigation.header_menu.map((item: { title: string, page: any, subMenu: Array<Object> }, index: number) => {

						return (
							<div
								onMouseEnter={() => hoverItem(index)}
								onMouseLeave={() => setActiveItem(null)}
								key={index} className={`relative ${activeItem == index ? "active" : ""}`}>
								{item.subMenu ? 
									<button
									className="text-[65px] w-full text-left flex menu_item_parent xl:max-w-[620px] border-b-[1px] border-solid border-[#30282A] py-[0px] text-[#30282A] uppercase tracking-[1.3px] leading-[96px] boing_thin"
									>{item.title}
								</button>
									: <Link
									className="text-[65px] block w-auto menu_item_parent xl:max-w-[620px] border-b-[1px] border-solid border-[#30282A] py-[0px] text-[#30282A] uppercase tracking-[1.3px] leading-[96px] boing_thin"
									href={`${item.page.slug.slug != null ? `/${item.page.slug.slug}` : "/"}`}>{item.title}
								</Link>
								}
			...
			>
				...
				<div className={`children relative ${openMenu ? "slideDown" : ""}`}>
					<div className='overflow-hidden min-h-[100vh] flex flex-col relative justify-between'>{children}</div>
				</div>
			</ThemeContext.Provider>
		</div>
	);

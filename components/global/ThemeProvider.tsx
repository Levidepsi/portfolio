...
export const ThemeProvider = ({
  ...
  const hideLanding = closeLanding ? "z-0 opacity-0 invisible" : "z-[999] opacity-[1] visible";

  useEffect(() => {
    const popUpClosed = localStorage.getItem("closePopup");
    if (popUpClosed !== "true" && !pathname.includes("/admin")) {
      setShowPopup(true);
      setShowOverlay(true);
    } 
    setLoading(false);
  ...
  useEffect(() => {
    const popupClosed = localStorage.getItem("closePopup") === "true";
    if (!popupClosed) {
        ...
        closeLanding !== true ? (
          <div
            onClick={() => {
              ...
              setTimeout(() => setHidePerm(true), 1000);
            }}
